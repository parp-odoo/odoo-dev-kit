 ...
 */
export async function findRemoteWithBranch(
	repoPath: string,
	branch: string,
): Promise<string | null> {
...
export async function checkoutBranch(repo: Repo, branch: string): Promise<void> {
	...
	try {
		...
		const isVersionAvailable = await findRemoteWithBranch(path, version);
		const isBranchAvailable = await findRemoteWithBranch(path, branch);
...
export async function remoteUpdate(repo: Repo): Promise<void> {
	...
	try {
		...
		throw new Error(
			`Remote update failed in ${path}: ${err.stderr || err.error?.message || err.message}`,
		);
	}
}
...
export async function createNewBranch(
	...
	try {
		...
		throw new Error(
			`New branch failed in ${path}: ${err.stderr || err.error?.message || err.message}`,
		);
	}
}
...
export async function pushBranch(repo: Repo, force: boolean = false): Promise<void> {
	...
	try {
		...
		throw new Error(
			`Push failed in ${path}: ${err.stderr || err.error?.message || err.message}`,
		);
	}
}
...
export async function commitChanges(repo: Repo, commitMessage: string): Promise<boolean> {
	...
	try {
		...
		throw new Error(
			`Commit failed in ${path}: ${err.stderr || err.error?.message || err.message}`,
		);
	}
}
...
export async function amendCommit(repo: Repo, commitMessage?: string): Promise<boolean> {
	...
	try {
		...
		throw new Error(
			`Amend failed in ${path}: ${err.stderr || err.error?.message || err.message}`,
		);
	}
}
...
export async function hasDiff(repo: Repo): Promise<boolean> {
	...
	try {
		...
		const { stdout: commitsStdout } = await execCommand(
			"git log @{u}..HEAD --oneline",
			path,
		).catch(() => ({ stdout: "no tracking" }));

		return (
			diffStdout.trim().length > 0 ||
			(commitsStdout.trim().length > 0 && commitsStdout !== "no tracking")
		);
	} catch (err) {
